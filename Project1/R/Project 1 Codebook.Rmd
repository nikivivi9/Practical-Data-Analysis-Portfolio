...
This project combines information from four datasets: `Project1.csv`, `aqi_values.csv`, `course_record.csv`, and `marathon_dates.csv`.

The primary dataset in this project, `Project1.csv`, consists of 11,564 observations with 14 variables, including information on participants’ characteristics and environmental condition characteristics collected from five major marathon races over a period of 15-20 years in the U.S.: the Boston Marathon, Chicago Marathon, New York City Marathon, Twin Cities Marathon (Minneapolis, MN), and Grandma’s Marathon (Duluth, MN). Our response variable is `CR_pct`, representing the percent off the current course record (%CR). Key covariates include race, sex, temperature (`Tdc`, `Twc`, `Tgc`, etc.), relative humidity (`rh`), solar radiation (`SRWm2`), dew point (`DP`), wind speed (`Wind`), and Wet Bulb Globe Temperature (`WBGT`). 

`aqi_values.csv` consists of 10,451 observations and 11 variables, containing detailed information on Air Quality Index (AQI) for each race day over years. `course_record.csv` has 194 rows and 4 columns, providing information on the best course record for each marathon race by gender. `marathon_dates.csv` includes information of specific date on which each races were held with 98 observations and 3 variables.

# Preprocessing

The main data set has 491 observations with missing values showing in Figure 1, and these missingness appears mainly among environmental conditions covariates from races held in 2011 and 2012. These missing observations were neither excluded from the analysis nor imputed to retain as much data as possible for exploration.
...
Moreover, we mutated a new column `age_group` for further aging analysis. Based on the age distribution in the dataset, runners were classified into five aging groups: Younger, Lower-Mid Age, Mid Age, Upper-Mid Age, and Highest Age with cut-off points presented in Table 1. These cut-off points are decided by considering both the natural breaks in the data and common practices in marathon age classifications. Instead of using the traditional 10-year groups, we set 15-year groups to better reflect the age distribution of participants in our dataset.
...
Additionally, in Figure 2, both genders' plots exhibit a U-shaped curve which further explains the relationship between age and performance. The performance continuously improves as age increases (%CR decreases as age increases), achieve the optimal performance (lowest %CR), and then gradually declines as age increases furthur (%CR increases as age increases). Both genders reach their optimal performance during their Lower-Mid age where female participants' average %CR achieves the lowest value at age of 28 and male participants achieves the lowest average %CR at age 30. After reaching their optimal performance, both genders exhibits sharp decline as age increases. Male participants show more extreme decline compared to female participants especially during their upper-mid and highest age, end with a maximum average percent off course record (CR%) exceeding 300. 
...
For further exploration, we fit a mixed-effects model with random intercept by age to present quantitative measures of significance. This approach helps us to account the various level of performance across different age groups we mentioned earlier. That is, younger, upper-mid, and highest age group runners tend to have worse performance and lower-mid and mid age group people are more likely to have greater performance. By using mixed-effects model with random intercept on age, we can capture the variation in runners' baseline performance variation among individuals across various age groups.

$$
\text{CR\_pct}_{ij} = \beta_0 + \beta_1 \cdot \text{rh}_i + \beta_2 \cdot \text{SRWm2}_i + \beta_3 \cdot \text{Wind}_i + \beta_4 \cdot \text{WBGT}_i + \beta_5 \cdot \text{aqi}_i + u_j + \epsilon_{ij}
$$


The mixed-effects model results are presented in Table 5 and Table 6. Among our environmental condition variables, `rh`, `SRWm2`, and `WBGT` exhibited statistical significance in our model with p-value less than 0.05 while `Wind` and `aqi` were not statistically significant with p-value larger than 0.05. WBGT had the largest and most significant effect on marathon performance compared to other factors, with an estimate of 0.5326 where p < 2e-16. This indicates that adjusting for other covariates, as WBGT increases by one unit, the %CR would increases by around 0.53% on average, leading to worse performance. 

Although `rh` and `SRWm2` showed significance, their coefficients were extremely small. To be noticed, the coefficient estimates of both two variables had negative estimates, indicating a weakly negative effect of solar radiation and relative humidity on runner's performance. This result may initially seems counterintuitive that we would expect higher values of these two factors might be more challenge for runners to complete the race. However, remind what we discuss in the environmental condition section, moderate levels of these variables may not be as harmful as expected. Also, environmental factors do interact with each other in reality. Although the two variables appears minor and negative effect on themselves, they might have more complex effects in shaping overall race conditions. 

```{r}
# fit a mixed-effects model with random intercept of age
model <- lmerTest::lmer(CR_pct ~ rh + SRWm2 + Wind + WBGT + aqi + (1 | age), data = data)

# extract the fixed and random from the model results
fixed <- tidy(model, effects = "fixed")
random <- tidy(model, effects = "ran_pars")

# Print random effects using kable
random[1,3] <- "Intercept"
random[, -1] %>%
  kbl(booktabs = TRUE, caption = "Random Effects of the Model",
      col.names = c("Group", "Term","SD"),
      longtable = TRUE, linesep = "") %>%
  kable_styling(font_size = 10,
                latex_options = c("repeat_header", "HOLD_position", "scale_down"))

# Print fixed effects using kable
fixed[, -c(1, 6)] %>%
